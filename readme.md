# 📚 Database Learning Roadmap

## Chapter 1 — Database Fundamentals

- [x] [1.1 — Database & Table](chapter-01/01-database-and-table.md)
- [x] [1.2 — Data Types](chapter-01/02-data-types.md)
- [x] [1.3 — NULL](chapter-01/03-null.md)
- [x] [1.4 — Constraints](chapter-01/04-constraints.md)
- [x] [1.5 — Primary Key & IDENTITY](chapter-01/05-primary-key-identity.md)
- [x] [1.6 — ALTER TABLE](chapter-01/06-alter-table.md)

## Chapter 2 — Querying & Data Modification

- [x] [2.1 — INSERT](chapter-02/01-insert.md)
- [x] [2.2 — SELECT](chapter-02/02-select.md)
- [x] [2.3 — WHERE](chapter-02/03-where.md)
- [x] [2.4 — ORDER BY](chapter-02/04-order-by.md)
- [x] [2.5 — TOP](chapter-02/05-top.md)
- [x] [2.6 — Aggregate Functions](chapter-02/06-aggregate-functions.md)
- [x] [2.7 — GROUP BY](chapter-02/07-group-by.md)
- [x] [2.8 — HAVING](chapter-02/08-having.md)
- [x] [2.9 — UPDATE & DELETE](chapter-02/09-update-and-delete.md)
- [x] [2.10 — T-SQL Variables](chapter-02/10-t-sql-variables.md)

## Chapter 3 — Relationships & JOIN

- [x] [3.1 — Foreign Key & Relationships](chapter-03/01-foreign-key-relationships.md)
- [x] [3.2 — INNER JOIN](chapter-03/02-inner-join.md)
- [x] [3.3 — LEFT JOIN & RIGHT JOIN](chapter-03/03-left-right-join.md)
- [x] [3.4 — FULL OUTER JOIN](chapter-03/04-full-outer-join.md)
- [x] [3.5 — CROSS JOIN](chapter-03/05-cross-join.md)
- [x] [3.6 — SELF JOIN](chapter-03/06-self-join.md)

## Chapter 4 — Database Design

- [x] [4.1 — One-to-One, One-to-Many & Many-to-Many](chapter-04/01-relationships.md)
- [ ] [4.2 — Normalization](chapter-04/02-normalization.md)
- [ ] [4.3 — 1NF, 2NF & 3NF](chapter-04/03-normal-forms.md)
- [ ] [4.4 — ERD](chapter-04/04-erd.md)

## Chapter 5 — Transactions

- [ ] [5.1 — Transaction Basics](chapter-05/01-transactions.md)
- [ ] [5.2 — COMMIT & ROLLBACK](chapter-05/02-commit-rollback.md)

## Chapter 6 — Essential Querying

- [ ] [6.1 — Subquery](chapter-06/01-subquery.md)
- [ ] [6.2 — EXISTS & NOT EXISTS](chapter-06/02-exists-not-exists.md)
- [ ] [6.3 — CASE](chapter-06/03-case.md)
- [ ] [6.4 — COALESCE](chapter-06/04-coalesce.md)
- [ ] [6.5 — UNION & UNION ALL](chapter-06/05-union.md)

## 🚀 Checkpoint

After Chapter 6:

I will return to databases after learning a few other things.